...
void z_arm64_el2_init(void)
{
	...
	/* when EL2 is enable in current security status:
		Clear TGE bit: All exceptions that would be routed to EL1 are routed to EL2;
		Clear IMO bit: Physical IRQ interrupts are taken to EL2, unless they are routed to EL3
	*/
	reg &= ~(HCR_IMO_BIT | HCR_TGE_BIT);
